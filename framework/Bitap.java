 ...
 * (C) 2006 <a
 * href="http://neil.fraser.name/software/diff_match_patch/">http://neil.fraser.name/software/diff_match_patch/</a>
 * 
...
public class Bitap implements Locator {
	...
	public int locate() {
		...
		// Coerce the text length between reasonable maximums and
		// minimums.
		...
		for (int d = 0; d < pattern.length(); d++) {
			...
			for (int j = finish - 1; j >= start; j--) {
				...
				int mask = alphabet.containsKey(curChar) ? ((Integer) alphabet
						.get(curChar)).intValue() : 0;
				...
				if ((rd[j] & matchmask) != 0) {
					...
					if (score <= scoreThreshold) {
						...
						if (j > loc) {
							...
							// Already passed loc, downhill from here on
							// in.
							break;
						}
					}
	...
	private double bitapScore(int e, int x) {
		...
		return (e / (float) pattern.length() / Bitap.balance)
				+ (d / (float) scoreTextLength / (1.0 - Bitap.balance));
	}
